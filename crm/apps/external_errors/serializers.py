...
class ExternalErrorCodeSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            "id", "group", "group_code", "group_name", "error_code",
            "error_name", "description", "keywords", "examples",
            "sort_order", "is_active", "created_at", "updated_at",
        ]
        read_only_fields = [
            "id", "group_code", "group_name", "created_at", "updated_at",
        ]

    def validate_group(self, group):
        if not group.is_active:
            raise serializers.ValidationError(
                "Không thể gán mã lỗi vào nhóm Inactive."
            )
        return group

    def validate_error_code(self, value):
        value = str(value or "").strip().upper()
        queryset = ExternalErrorCode.objects.filter(error_code__iexact=value)
        if self.instance:
            queryset = queryset.exclude(pk=self.instance.pk)
        if queryset.exists():
            raise serializers.ValidationError("Mã lỗi đã tồn tại.")
        return value
...
class ExternalErrorGroupSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            "id", "group_code", "group_name", "description", "sort_order",
            "is_active", "error_code_count", "created_at", "updated_at",
        ]
        read_only_fields = [
            "id", "error_code_count", "created_at", "updated_at",
        ]

    def get_error_code_count(self, obj):
        value = getattr(obj, "error_code_count", None)
        return value if value is not None else obj.error_codes.count()

    def validate_group_code(self, value):
        value = str(value or "").strip().upper()
        queryset = ExternalErrorGroup.objects.filter(group_code__iexact=value)
        if self.instance:
            queryset = queryset.exclude(pk=self.instance.pk)
        if queryset.exists():
            raise serializers.ValidationError("Mã nhóm lỗi đã tồn tại.")
        return value


class ExternalErrorCauseGroupSerializer(serializers.ModelSerializer):
    record_count = serializers.SerializerMethodField()

    class Meta:
        model = ExternalErrorCauseGroup
        fields = [
            "id", "cause_code", "cause_name", "description", "keywords",
            "examples", "sort_order", "is_active", "record_count",
            "created_at", "updated_at",
        ]
        read_only_fields = [
            "id", "record_count", "created_at", "updated_at",
        ]

    def get_record_count(self, obj):
        value = getattr(obj, "record_count", None)
        return value if value is not None else obj.records.count()

    def validate_cause_code(self, value):
        value = str(value or "").strip().upper()
        queryset = ExternalErrorCauseGroup.objects.filter(
            cause_code__iexact=value
        )
        if self.instance:
            queryset = queryset.exclude(pk=self.instance.pk)
        if queryset.exists():
            raise serializers.ValidationError("Mã nguyên nhân đã tồn tại.")
        return value
...
class ExternalErrorImportBatchSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            "id", "batch_code", "file_name", "source_type", "total_rows",
            "classified_rows", "failed_rows", "status", "created_by",
...
class ExternalErrorRecordSerializer(serializers.ModelSerializer):
    ...
    error_code_value = serializers.CharField(
        source="error_code.error_code", read_only=True, allow_null=True
    )
    error_code_name = serializers.CharField(
        source="error_code.error_name", read_only=True, allow_null=True
    )
    error_group_id = serializers.IntegerField(
        source="error_code.group_id", read_only=True, allow_null=True
    )
    error_group_code = serializers.CharField(
        source="error_code.group.group_code", read_only=True, allow_null=True
    )
    error_group_name = serializers.CharField(
        source="error_code.group.group_name", read_only=True, allow_null=True
    )
    ...
    cause_group_code = serializers.CharField(
        source="cause_group.cause_code", read_only=True, allow_null=True
    )
    cause_group_name = serializers.CharField(
        source="cause_group.cause_name", read_only=True, allow_null=True
    )
    ...
    class Meta:
        ...
        fields = [
            ...
            "classification_status", "classification_error", "llm_model_id",
            "classified_at", "cause_group", "cause_group_code",
            "cause_group_name", "normalized_cause",
            "cause_classification_confidence", "cause_need_review",
            "cause_classification_status", "cause_classification_error",
...
class ExternalErrorRecordListSerializer(ExternalErrorRecordSerializer):
    class Meta(ExternalErrorRecordSerializer.Meta):
        fields = [
            ...
            "cause_need_review", "cause_classification_status",
            "cause_classification_error", "cause_classified_at", "created_at",
        ]
        read_only_fields = fields
...
class ExternalErrorManualCreateSerializer(serializers.Serializer):
    ...
    def validate_received_date(self, value):
        if parse_received_datetime(value) is None:
            raise serializers.ValidationError("Ngày nhận không đúng định dạng.")
        return value
...
class ExternalErrorConfirmCauseSerializer(serializers.Serializer):
    cause_group = serializers.PrimaryKeyRelatedField(
        queryset=ExternalErrorCauseGroup.objects.filter(is_active=True)
    )
    normalized_cause = serializers.CharField(
        required=False, allow_blank=True, allow_null=True, max_length=255
    )
...
class ExternalErrorDashboardWidgetSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = [
            "id", "title", "widget_type", "group_by", "breakdown_by",
            "metric", "sort_by", "sort_direction", "limit", "filters",
