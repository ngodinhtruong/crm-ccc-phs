...
def get_period_metric_by_code(period, profile, metric_code):
...
def calculate_min_calls_rate(period, profile, user, records_queryset):
    total_calls = records_queryset.count()

    sa_profile = KpiProfile.objects.filter(
        period=period,
    ...
    if sa_profile:
        target_metric = get_period_metric_by_code(period, sa_profile, "B1_17")

    target_calls = Decimal("0")

    if target_metric:
        target_calls = get_user_target_value(period, sa_profile, target_metric, user) or Decimal("0")
...
def calculate_gate_actual_value(period, profile, user, gate_config, records_queryset):
    formula_key = gate_config.formula_key

    if formula_key == "min_calls_rate":
        return calculate_min_calls_rate(period, profile, user, records_queryset)
...
def calculate_gates_for_user(*, period, profile, user, calculated_by_user=None):
    ...
    gate_configs = KpiPeriodGateConfig.objects.filter(
        ...
        profile=profile,
        is_active=True,
    ).select_related("gate_definition", "profile")
...
def get_group_weight(profile, group_type):
    return (
...
def get_user_metric_score_sum(period, profile, user, source_type):
    return (
...
def get_failed_gate_codes(period, profile, user):
...
def all_required_gates_passed(period, profile, user):
    ...
    return required_gate_count == passed_gate_count


def save_user_summary(*, period, profile, user, calculated_by_user=None):
    employee = getattr(user, "employee", None)
    branch = getattr(employee, "branch", None) if employee else None
    ...
    manual_score = get_user_metric_score_sum(
        ...
        profile,
        user,
        KpiUserMetricResult.SOURCE_MANUAL,
    )
    auto_score = get_user_metric_score_sum(
        period,
        profile,
        user,
        KpiUserMetricResult.SOURCE_AUTO,
    )
    total_score = manual_score + auto_score

    manual_weight = get_group_weight(profile, KpiGroup.GROUP_TYPE_MANUAL)
    auto_weight = get_group_weight(profile, KpiGroup.GROUP_TYPE_AUTO)
    failed_gate_codes = get_failed_gate_codes(period, profile, user)
    gates_passed = all_required_gates_passed(period, profile, user)
...
def get_metric_actual_value(period, profile, user, rank_metric_code):
    metric_codes = RANK_METRIC_CODE_MAP.get(rank_metric_code, [rank_metric_code])
    ...
    result = (
        KpiUserMetricResult.objects.filter(
            ...
            profile=profile,
            user=user,
            metric__metric_code__in=metric_codes,
        )
        .order_by("-id")
        .first()
    )
...
def calculate_overall_rank_map(period, profile):
    summaries = list(
        KpiUserSummary.objects.filter(period=period, profile=profile).order_by("-total_score", "id")
    )
...
def calculate_branch_rank_map(period, profile):
    rank_map = {}
    branch_ids = (
        KpiUserSummary.objects.filter(period=period, profile=profile)
...
def calculate_metric_rank_map(period, profile, rank_metric_code):
    summaries = list(KpiUserSummary.objects.filter(period=period, profile=profile).order_by("id"))
    rows = []
    ...
    for summary in summaries:
        rows.append(
            {
                "summary": summary,
                "value": get_metric_actual_value(period, profile, summary.user, rank_metric_code),
            }
        )
...
def get_reward_tier(period, profile, summary):
    tiers = {
        tier.tier_code: tier
...
def update_ranks_and_rewards(period, profile, calculated_by_user=None):
    overall_rank_map = calculate_overall_rank_map(period, profile)
    branch_rank_map = calculate_branch_rank_map(period, profile)
    fee_rank_map = calculate_metric_rank_map(period, profile, "TOTAL_FEE")
    reactivated_rank_map = calculate_metric_rank_map(period, profile, "REACTIVATED_ACCOUNTS")

    summaries = KpiUserSummary.objects.filter(period=period, profile=profile).select_related("user")
    ...
    for summary in summaries:
        ...
        reward_tier = get_reward_tier(period, profile, summary)
...
def user_matches_profile(user, profile):
    ...
    return user.user_roles.filter(role__role_code=profile.target_role_code).exists()


def get_profiles_for_calculation(period, profile=None):
    if profile:
        return [profile]

    return list(KpiProfile.objects.filter(period=period, is_active=True).order_by("sort_order", "id"))
...
def calculate_kpi_summaries(
    *,
    ...
    summaries = []
    profiles = get_profiles_for_calculation(period, profile=profile)

    for current_profile in profiles:
        if users is None:
            target_users = list(get_default_kpi_users(profile=current_profile, branch_id=branch_id))
        else:
            target_users = [item for item in users if user_matches_profile(item, current_profile)]
        ...
        for target_user in target_users:
            ...
            summaries.append({"summary": summary, "created": created})

        update_ranks_and_rewards(period, current_profile, calculated_by_user=calculated_by_user)
