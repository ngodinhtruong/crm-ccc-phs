  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={trends} barCategoryGap="40%">
        ...
        <XAxis dataKey="date" tick={{ fontSize: 11 }} padding={{ left: 20, right: 20 }} />
        <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
        <Tooltip formatter={(v: any) => [v, "Số lượng"]} />
        <Bar dataKey="count" name="Tổng số lượng" fill={C_GREEN2} radius={[3, 3, 0, 0]} isAnimationActive={false}>
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={trends} barCategoryGap="30%">
        ...
        <Bar dataKey="lien_he_thanh_cong" name="Liên hệ KH thành công" fill={C_GREEN} radius={[3, 3, 0, 0]} isAnimationActive={false}>
          <LabelList dataKey="lien_he_thanh_cong" position="top" style={{ fontSize: 10, fontWeight: 700, fill: C_GREEN }} />
        </Bar>
        <Bar dataKey="khong_lien_he_duoc" name="Không liên hệ được KH" fill={C_ORANGE} radius={[3, 3, 0, 0]} isAnimationActive={false}>
          <LabelList dataKey="khong_lien_he_duoc" position="top" style={{ fontSize: 10, fontWeight: 700, fill: C_ORANGE }} />
        </Bar>
        <Bar dataKey="khac_lien_he" name="Khác" fill={C_BLUE} radius={[3, 3, 0, 0]} isAnimationActive={false}>
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <ComposedChart data={surveyTrends} barCategoryGap="40%">
        ...
        <Tooltip />
        <Legend wrapperStyle={{ fontSize: 11 }} />
        <Bar dataKey="called" name="Gọi khảo sát" fill={C_SURVEY_GREEN} radius={[3, 3, 0, 0]} isAnimationActive={false}>
          <LabelList dataKey="called" position="top" style={{ fontSize: 10, fontWeight: 700, fill: C_SURVEY_GREEN }} />
        </Bar>
        <Bar dataKey="not_called" name="Không gọi" fill={C_SURVEY_RED} radius={[3, 3, 0, 0]} isAnimationActive={false}>
...
// ──────────────────────────────────────────
// ──────────────────────────────────────────
// 4. Kết quả kết nối (ComposedChart: Bar + Line xu hướng kết nối & tỉ lệ %)
// ──────────────────────────────────────────
function ChartKetQuaKetNoi({ trends }: { trends: EkycDailyTrend[] }) {
  if (!trends.length) return <p className="text-center text-xs text-slate-400 py-8">Không có dữ liệu</p>;
  
  const lineTrends = trends.map((t) => {
    const success = Number(t.ket_noi_thanh_cong ?? 0);
    const fail = Number(t.khong_ket_noi_duoc ?? 0);
    const total = success + fail;
    const rate = total > 0 ? Number(((success / total) * 100).toFixed(1)) : 0;
    return {
      ...t,
      ket_noi_thanh_cong: success,
      khong_ket_noi_duoc: fail,
      rate,
    };
  });
  ...
  return (
    <ResponsiveContainer width="100%" height={230}>
      <ComposedChart data={lineTrends} barCategoryGap="30%">
        <CartesianGrid vertical={false} strokeDasharray="3 3" stroke="#e2e8f0" />
        <XAxis dataKey="date" tick={{ fontSize: 11 }} padding={{ left: 20, right: 20 }} />
        <YAxis yAxisId="left" tick={{ fontSize: 11 }} allowDecimals={false} />
        <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 11 }} unit="%" domain={[0, 100]} />
        <Tooltip formatter={(val: any, name: any) => [name === "Tỷ lệ kết nối thành công (%)" ? `${val}%` : val, name]} />
        <Legend wrapperStyle={{ fontSize: 11 }} />
        <Bar yAxisId="left" dataKey="ket_noi_thanh_cong" name="Kết nối KH thành công" fill={C_GREEN} radius={[3, 3, 0, 0]} isAnimationActive={false}>
          <LabelList dataKey="ket_noi_thanh_cong" position="top" style={{ fontSize: 10, fontWeight: 700, fill: C_GREEN }} />
        </Bar>
        <Bar yAxisId="left" dataKey="khong_ket_noi_duoc" name="Không kết nối được KH" fill={C_ORANGE} radius={[3, 3, 0, 0]} isAnimationActive={false}>
...
// ──────────────────────────────────────────
// 5. Xu hướng kết quả kết nối (Line Chart - dạng %)
// ──────────────────────────────────────────
function ChartXuHuongKetNoiLine({ trends }: { trends: EkycDailyTrend[] }) {
  if (!trends.length) return <p className="text-center text-xs text-slate-400 py-8">Không có dữ liệu</p>;

  const percentTrends = trends.map((t) => {
    const success = Number(t.ket_noi_thanh_cong ?? 0);
    const fail = Number(t.khong_ket_noi_duoc ?? 0);
    const total = success + fail;
    const rateSuccess = total > 0 ? Number(((success / total) * 100).toFixed(1)) : 0;
    const rateFail = total > 0 ? Number(((fail / total) * 100).toFixed(1)) : 0;
    return {
      ...t,
      rateSuccess,
      rateFail,
  ...
  return (
    <ResponsiveContainer width="100%" height={230}>
      <ComposedChart data={percentTrends}>
        <CartesianGrid vertical={false} strokeDasharray="3 3" stroke="#e2e8f0" />
        <XAxis dataKey="date" tick={{ fontSize: 11 }} padding={{ left: 25, right: 25 }} />
        ...
        <Line
          ...
          stroke={C_GREEN}
          strokeWidth={3}
          dot={{ fill: C_GREEN, r: 5, stroke: "#fff", strokeWidth: 2 }}
          activeDot={{ r: 7 }}
          isAnimationActive={false}
        >
        ...
        <Line
          type="monotone"
          dataKey="rateFail"
          name="% Không kết nối được"
          stroke={C_ORANGE}
          strokeWidth={3}
          dot={{ fill: C_ORANGE, r: 5, stroke: "#fff", strokeWidth: 2 }}
          activeDot={{ r: 7 }}
          isAnimationActive={false}
        >
...
// ──────────────────────────────────────────
// 6. Kết quả KH phản hồi (ComposedChart: 4 Bar series + Line tổng)
// ──────────────────────────────────────────
function ChartKetQuaKhPhanHoi({ trends }: { trends: EkycDailyTrend[] }) {
  if (!trends.length) return <p className="text-center text-xs text-slate-400 py-8">Không có dữ liệu</p>;

  const responseTrends = trends.map((t) => {
    const b = Number(t.bam_phim ?? 0);
    const kb = Number(t.khong_bam_phim ?? 0);
    const tm = Number(t.tat_may_ngang ?? 0);
    const kn = Number(t.khong_ket_noi_result ?? 0);
    const total = b + kb + tm + kn;
    return {
      ...t,
      bam_phim: b,
      khong_bam_phim: kb,
      tat_may_ngang: tm,
      khong_ket_noi_result: kn,
      total_response: total > 0 ? total : Number(t.count ?? 0),
    };
  });

  return (
    <ResponsiveContainer width="100%" height={230}>
      <ComposedChart data={responseTrends} barCategoryGap="20%">
        <CartesianGrid vertical={false} strokeDasharray="3 3" stroke="#e2e8f0" />
        <XAxis dataKey="date" tick={{ fontSize: 11 }} padding={{ left: 20, right: 20 }} />
        <YAxis tick={{ fontSize: 11 }} allowDecimals={false} />
        <Tooltip />
        <Legend wrapperStyle={{ fontSize: 11 }} />
        <Bar dataKey="bam_phim" name="KH bấm nút 1 (Không gặp trở ngại)" fill={C_SURVEY_GREEN} radius={[3, 3, 0, 0]} isAnimationActive={false}>
          <LabelList dataKey="bam_phim" position="top" style={{ fontSize: 10, fontWeight: 700, fill: C_SURVEY_GREEN }} />
        </Bar>
        <Bar dataKey="khong_bam_phim" name="KH không bấm nút" fill={C_RED} radius={[3, 3, 0, 0]} isAnimationActive={false}>
          <LabelList dataKey="khong_bam_phim" position="top" style={{ fontSize: 10, fontWeight: 700, fill: C_RED }} />
        </Bar>
        <Bar dataKey="tat_may_ngang" name="KH Tắt máy ngang" fill={C_GREY} radius={[3, 3, 0, 0]} isAnimationActive={false}>
          <LabelList dataKey="tat_may_ngang" position="top" style={{ fontSize: 10, fontWeight: 700, fill: C_GREY }} />
        </Bar>
        <Bar dataKey="khong_ket_noi_result" name="KH không kết nối" fill={C_LIME} radius={[3, 3, 0, 0]} isAnimationActive={false}>
          <LabelList dataKey="khong_ket_noi_result" position="top" style={{ fontSize: 10, fontWeight: 700, fill: C_LIME }} />
        </Bar>
        <Line
          type="monotone"
          dataKey="total_response"
          name="Tổng phản hồi"
          stroke="#0891b2"
          strokeWidth={3}
          dot={{ fill: "#0891b2", r: 5, stroke: "#fff", strokeWidth: 2 }}
          ...
          isAnimationActive={false}
        >
          <LabelList dataKey="total_response" position="top" style={{ fontSize: 10, fontWeight: 700, fill: "#0891b2" }} />
        </Line>
      </ComposedChart>
    </ResponsiveContainer>
  );
...
export function EkycDashboardCharts({ trends }: EkycChartsProps) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <ChartCard title="Xu hướng kết quả kết nối">
        <ChartXuHuongKetNoiLine trends={trends} />
      </ChartCard>
