    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-900/45 p-4 backdrop-blur-sm">
      <div className="max-h-[86vh] w-full max-w-5xl overflow-hidden rounded-lg border border-slate-200 bg-white shadow-2xl">
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 bg-slate-50/80 px-5 py-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-[#0097cf]">Chi tiết tài khoản kích hoạt</p>
          ...
          <table className="w-full min-w-[900px] border-collapse text-left text-xs">
            ...
            <tbody>
              ...
              {employee.accounts.map((account, index) => (
                <tr key={`${account.account_no}-${index}`} className={`h-12 border-b border-slate-100 ${index % 2 === 0 ? "bg-white" : "bg-slate-50/50"}`}>
                  <td className="px-3 font-semibold text-[#007ead]">{account.account_no}</td>
                  <td className="px-3 text-slate-700">{account.customer_name || "-"}</td>
                  <td className="px-3 text-slate-600">{account.branch_name || "-"}</td>
                  <td className="px-3 text-right font-semibold text-amber-600" title={formatMoney(account.transaction_fee)}>{formatCompactMoney(account.transaction_fee)}</td>
                  <td className="px-3 text-right text-slate-600" title={formatMoney(account.transaction_value)}>{formatCompactMoney(account.transaction_value)}</td>
                  <td className="px-3 text-right">{formatNumber(account.order_count)}</td>
                  <td className="px-3 text-slate-500">{account.call_date || "-"}</td>
                </tr>
              ))}
            </tbody>
          </table>
